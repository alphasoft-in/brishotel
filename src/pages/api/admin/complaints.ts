...
export const DELETE: APIRoute = async ({ request, cookies }) => {
    ...
    try {
        ...
        if (!id) {
            return new Response(JSON.stringify({ success: false, error: "Falta ID de reclamación" }), { status: 400, headers: jsonHeaders });
        }
        ...
        if (success) {
            ...
            return new Response(JSON.stringify({ success: false, error: "Error al eliminar la reclamación" }), { status: 500, headers: jsonHeaders });
        }
